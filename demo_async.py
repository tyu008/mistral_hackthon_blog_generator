...
The new section starts with the section title, following by a list of sub-sections.
DO not conlcude anything in the new section.
The generate new sections starts with '##' in an organized markdown format with a list of sub-sections.
...
async def main():
    ...
    for i in range(2, len(subsecs)-1):
        ...
        img = retrieve_image(section_title)
        if img:
            img_urls.append(img)
